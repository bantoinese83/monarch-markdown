...
const FindReplace: React.FC<FindReplaceProps> = ({
  ...
  useEffect(() => {
    ...
    if (storedHistory) {
      try {
        ...
        // Failed to parse find history from localStorage
      }
    }
  }, []);
  ...
  useEffect(() => {
    if (show) {
      // Auto-focus the find input when opened
      findInputRef.current?.focus();
      findInputRef.current?.select();

      if (findTerm) {
        const newCurrentIndex = matches.findIndex(
          (match) => match.index >= (editorRef.current?.selectionStart ?? 0)
        );
  ...
  const handleReplace = () => {
    ...
    const newMarkdown =
      markdown.substring(0, index) + replaceTerm + markdown.substring(index + length);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !findInputRef.current.contains(event.target as Node) &&
  ...
  const filteredHistory = useMemo(() => {
    ...
    return searchHistory.filter(
      (term) =>
        term.toLowerCase().includes(findTerm.toLowerCase()) &&
        term.toLowerCase() !== findTerm.toLowerCase()
    );
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-grow">
            ...
            {showSuggestions && filteredHistory.length > 0 && (
              <div
                ref={suggestionsRef}
                className="absolute top-full left-0 right-0 mt-1 bg-monarch-bg border border-monarch-light/50 rounded-md shadow-lg z-30 max-h-40 overflow-y-auto"
              >
                {filteredHistory.map((term: string) => (
                  <div
                    key={term}
                    className="px-3 py-2 text-sm hover:bg-monarch-main cursor-pointer"
                    onMouseDown={(e) => {
                      e.preventDefault();
                      setFindTerm(term);
                      setShowSuggestions(false);
