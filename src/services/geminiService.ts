...
import { GeminiError } from '@/src/errors';
import { retryWithBackoff, validateText } from '@/src/utils';
  ...
  return retryWithBackoff(async () => {
    try {
      ...
      throw new GeminiError(
        ...
        'Failed to communicate with the Gemini API. Please try again.'
      );
    }
  });
};
  ...
  return retryWithBackoff(async () => {
    try {
      ...
      throw new GeminiError(
        ...
        'Failed to generate image from the Gemini API. Please try again.'
      );
    }
  });
};
...
export const generateSpeech = async (text: string): Promise<string> => {
  ...
  try {
    ...
    throw new GeminiError(
      ...
      'Failed to communicate with the Gemini TTS API. Please try again.'
    );
  }
};
