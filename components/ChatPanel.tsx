...
const ChatPanel: React.FC<ChatPanelProps> = ({ isOpen, onClose, tools, addToast }) => {
          ...
          if (chunk.functionCalls) {
            ...
            await Promise.all(
              calls.map(async (call) => {
                ...
                // Note: Function responses handling may need API-specific implementation
                // For now, we'll continue with the stream
                return {
                  id: callId,
                  name: callName,
            ...
            // Continue with the stream - function responses may need different handling
            // This is a placeholder for proper function response handling
          } else {
  ...
  return (
    <aside
      className={`flex-shrink-0 bg-white dark:bg-monarch-bg-light border-l border-gray-200 dark:border-monarch-main z-20 flex flex-col transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'w-[400px]' : 'w-0'}`}
    >
      <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-monarch-main flex-shrink-0">
        <h2 className="text-lg font-bold flex items-center gap-2">
          <BotIcon className="w-6 h-6 text-monarch-accent" /> AI Assistant
        </h2>
        <button
          onClick={onClose}
          className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-monarch-main"
        >
      ...
      <div className="p-3 border-t border-gray-200 dark:border-monarch-main flex-shrink-0">
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-2 bottom-2 p-2 rounded-lg bg-monarch-accent hover:bg-monarch-accent-hover text-white disabled:bg-monarch-light/50 transition-colors"
          >
