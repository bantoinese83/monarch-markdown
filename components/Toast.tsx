...
const Toast: React.FC<ToastProps> = ({ toast, onDismiss }) => {
  ...
  return (
    <div
      ...
      role="alert"
    >
      <div className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-lg ${isSuccess ? 'bg-green-100 dark:bg-green-900/50 text-green-500 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/50 text-red-500 dark:text-red-300'}`}>
      ...
      <div className="ms-3 text-sm font-normal text-gray-700 dark:text-monarch-text-dark">{toast.message}</div>
    </div>
  );
};
