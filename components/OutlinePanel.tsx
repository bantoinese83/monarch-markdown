...
const OutlinePanel: React.FC<OutlinePanelProps> = ({ isOpen, onClose, outline }) => {
  ...
  return (
    <aside
      className={`flex-shrink-0 bg-white dark:bg-monarch-bg-light border-r border-gray-200 dark:border-monarch-main z-20 flex flex-col transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'w-[300px]' : 'w-0'}`}
    >
      <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-monarch-main flex-shrink-0">
        <h2 className="text-lg font-bold flex items-center gap-2">
          <ListTreeIcon className="w-6 h-6 text-monarch-accent" /> Document Outline
        </h2>
        <button
          onClick={onClose}
